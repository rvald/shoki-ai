...
router = APIRouter()
log = logging.getLogger("privacy.events")
...
REDACT_COMPLETED_TOPIC = os.getenv("REDACT_COMPLETED_TOPIC", "redact-completed")
...
if PUBSUB_ENABLED:
    ...
    TOPIC_PATH = publisher.topic_path(PROJECT_ID, REDACT_COMPLETED_TOPIC) if PROJECT_ID else None
...
async def _publish_completed(event: Dict[str, Any]) -> None:
    ...
    if not (publisher and PROJECT_ID and REDACT_COMPLETED_TOPIC):
...
def _enqueue_task(task_payload: Dict[str, Any]) -> None:
    """
    Enqueue a Cloud Task to POST /tasks/redact with a JSON body.
    """
    ...
    url = (TASKS_SERVICE_URL or "") + "/tasks/redact"  # if TASKS_SERVICE_URL empty, relative URL is fine on Cloud Run
...
async def pubsub_push(request: Request, background: BackgroundTasks):
    ...
    try:
        if tasks_client and TASKS_QUEUE:
            ...
            background.add_task(_process_redaction_task, task_payload)
    except Exception as e:
...
async def tasks_transcribe(task_body: Dict[str, Any]):
    """
    Cloud Task worker. Does the actual redaction and publishes *.completed.
    """
    ...
    try:
        await _process_redaction_task(task_body)
        return {"ok": True}
    except Exception as e:
...
async def _process_redaction_task(task_body: Dict[str, Any]) -> None:
    ...
    resp = redact_with_idempotency(req, corr, idem_key)
